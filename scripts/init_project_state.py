...
SCHEMA = STATE / "project_ledger_schema.sql"
...
state = {
    ...
    "current_branch": "phase1-final",
    "validation": {"unit_tests": "13/13 PASS", "compileall": "PASS", "static_secret_scan": "PASS", "static_external_io_scan": "PASS", "mixed_fuzz_benchmark": "100000 randomized requests, ~23882 decisions/sec", "benchmark_scope": "Phase 1 core only, not end-to-end WAF"},
    "done": ["canonical request/data models", "contracts/interfaces", "versioned baseline HTTP feature extraction", "deterministic decision policy", "event schema", "bounded body handling", "phase 1 tests", "architecture documentation", "portable SQLite project ledger"],
...
JSON.write_text(json.dumps({**state, "validated_at": now}, indent=2) + "\n", encoding="utf-8")
...
conn.execute("DELETE FROM tasks WHERE phase IN (1,2)")
...
conn.execute("INSERT INTO test_runs(phase,name,status,score,details,run_at) VALUES(?,?,?,?,?,?)", (1, "MASTER PHASE 1 GATE", "PASS", 10.0, "13/13 unit tests, compileall, static scans and mixed fuzz benchmark all passed.", now))
