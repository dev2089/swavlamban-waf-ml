...
RULE_SCHEMA_VERSION='rule-v1'; DEPLOYMENT_SCHEMA_VERSION='deployment-v1'
...
_SAFE_ACTIONS = {"block"}; _RULE_ID=re.compile(r'^WAF-ML-[A-Z0-9-]+-[0-9A-F]{8}$')
...
class RuleLifecycleManager:
        ...
        with self._lock:
            ...
            for row in evidence.detector_contributions:
                ...
                for feature,(label,prefix) in FEATURE_RULES.items():
                    ...
                    conf=min(.99,max(self.min_confidence,.70+contribution*.30+float(row.get('confidence',0))*.20+attribution*.20)); matcher={'feature':feature,'operator':'>=','threshold':1.0}; rid=f'{prefix}-{_fingerprint(matcher,"block")}'
                    if rid in self._rules: continue
                    rule=RuleRecord(rid,f'ML-derived {label} rule',f'Generated from Phase 5 decision evidence when {feature} was active; deployment remains human-approved.','feature_threshold',matcher,'block',round(conf,6),'phase5-decision-evidence',evidence.request_id,detector,tuple(sorted(evidence.rule_ids))); self._rules[rid]=rule; out.append(rule); self._audit.append({'event':'rule_generated','rule_id':rid,'status':rule.status.value,'source':rule.source,'source_request_id':rule.source_request_id,'at':rule.created_at})
        return out
    ...
    def rollback(self,did):
        with self._lock:
            ...
            idx=self._deployments.index(target); previous=self._deployments[idx-1] if idx else None; restore=(previous.active_rule_ids if previous else ()) if idx==len(self._deployments)-1 else target.active_rule_ids
    ...
    def match(self,features):
        return tuple(r.rule_id for r in self.active_rules() if r.matcher.get('operator')=='>=' and float(features.values.get(r.matcher.get('feature'),0))>=float(r.matcher.get('threshold',1)))
