...
class EdgeDecisionPolicy:
    """Deterministic policy combining known signatures and independent ML risk signals."""
    ...
    def decide(self, request: RequestEnvelope, signals: tuple[DetectionSignal, ...], pipeline_version: str) -> DecisionResult:
        ...
        if any(s.detector == "ml-runtime-failure" for s in signals):
            risk = 1.0
        elif signature is not None and signature.rule_ids:
            risk = 1.0
        else:
            weighted = sum(self._WEIGHTS.get(s.detector, 0.0) * s.score for s in signals)
            strongest = max((s.score for s in signals if s.detector != "open-source-waf-rules"), default=0.0)
            risk = strongest * 0.35 + weighted * 0.65
        ...
        return DecisionResult(
            decision=decision, risk_score=round(min(1.0, max(0.0, risk)), 6),
            reasons=reasons, rule_ids=rule_ids, signals=signals,
            request_id=request.request_id, pipeline_version=pipeline_version,
        )
