...
class BehaviouralDetector:
    """Learned per-source request-burst detector. Runtime state is process-local and bounded."""
    ...
    @staticmethod
    def _summary_features(count: int, unique_paths: int, span_seconds: float, mean_interval: float, interval_std: float) -> list[float]:
        rate = count / 10.0
    ...
    def _window_features(self, events: deque[tuple[float, str]]) -> list[float]:
        count = len(events)
        if count <= 1:
            span, mean_interval, interval_std = self.window_seconds, self.window_seconds, 0.0
        else:
    ...
    def detect(self, request: RequestEnvelope, features: FeatureVector) -> DetectionSignal:
        ...
        reason = (
            f"learned behavioural anomaly: {count} requests/{self.window_seconds:g}s, {unique_paths} unique paths"
            if score >= 0.5 else "learned behaviour within current baseline window"
        )
        return DetectionSignal(
            ...
            metadata={
                ...
                "state_scope": "process-local-bounded",
            },
        )
