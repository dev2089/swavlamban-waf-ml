...
5. `state/project_ledger.db` when generated by CI, otherwise `state/phase8_ledger.sql`
...
- `scripts/record_phase8_ledger.py`
...
python scripts/record_phase8_ledger.py
...
## Still open
Live Supabase application/migration, production FastAPI endpoint auth wiring, ModSecurity/Coraza, TLS/HTTPS external deployment, full load/failure testing, challenge evidence, dashboard, five-minute demo, final documentation/report and final release gate.

## Continuation rule
Preserve every prior milestone and every failure/remediation record. Never treat documentation or migration SQL as proof of live deployment. Never weaken human approval, no-auto-promotion or raw-request privacy boundaries.
