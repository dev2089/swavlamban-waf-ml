...
cleanup() {
  ...
  rm -f "$UP_LOG" "$WAF_LOG" /tmp/swavlamban-waf-access.log /tmp/swavlamban-waf-error.log /tmp/waf-block-response /tmp/waf-health-headers /tmp/swavlamban-waf-phase2.pid
}
trap cleanup EXIT
...
for i in $(seq 1 40); do
  if curl -fsS http://127.0.0.1:18080/__waf/health >/dev/null 2>&1; then break; fi
  sleep 0.1
done
curl -fsS http://127.0.0.1:18080/__waf/health >/dev/null
...
for i in $(seq 1 40); do
  if curl -fsS -D /tmp/waf-health-headers -o /dev/null http://127.0.0.1:18088/health 2>/dev/null; then break; fi
  sleep 0.1
done
allow_headers="$(cat /tmp/waf-health-headers)"
grep -q 'HTTP/1.1 200' <<<"$allow_headers"
grep -qi 'X-WAF-Decision: allow' <<<"$allow_headers"
